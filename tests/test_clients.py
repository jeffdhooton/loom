from loom.clients import make_judge_client, OLLAMA_BASE


def test_judge_client_local_points_at_ollama(monkeypatch):
    monkeypatch.delenv("LOOM_JUDGE_BASE_URL", raising=False)
...
def test_judge_client_respects_env_override(monkeypatch):
    monkeypatch.setenv("LOOM_JUDGE_BASE_URL", "http://127.0.0.1:8000/v1")
    c = make_judge_client("qwen3.6:27b")
